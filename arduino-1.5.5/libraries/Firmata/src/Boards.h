...
// Arduino DUE
#elif defined(__SAM3X8E__)  //|| defined(STM32F40_41xxx)
...
#define TOTAL_PINS              42 // 20 digital + 14 analog
#define VERSION_BLINK_PIN       41 // RED LED
#define IS_PIN_DIGITAL(p)       ((p)>=3&&(p)<TOTAL_PINS) // ((p) >= 3 && (p) < 19)||((p) >= 22 && (p) < TOTAL_PINS)
#define IS_PIN_ANALOG(p)        ((p)>=15&&(p)<TOTAL_PINS)// ((p) >= 15 && (p) <= 18) ||((p) >= 30 && (p) <= 39)
#define IS_PIN_PWM(p)           digitalPinHasPWM(p) 
#define PIN_TO_DIGITAL(p)       (p)
...
static inline unsigned char writePort(byte port, byte value, byte bitmask)
{
	...
	if ((bitmask & 0x01)) digitalWrite(PIN_TO_DIGITAL(pin+0), a0);
	if ((bitmask & 0x02)) digitalWrite(PIN_TO_DIGITAL(pin+1), a1);
	if ((bitmask & 0x04)) digitalWrite(PIN_TO_DIGITAL(pin+2), a2);
	if ((bitmask & 0x08)) digitalWrite(PIN_TO_DIGITAL(pin+3), a3);
	if ((bitmask & 0x10)) digitalWrite(PIN_TO_DIGITAL(pin+4), a4);
	if ((bitmask & 0x20)) digitalWrite(PIN_TO_DIGITAL(pin+5), a5);
	if ((bitmask & 0x40)) digitalWrite(PIN_TO_DIGITAL(pin+6), a6);
	if ((bitmask & 0x80)) digitalWrite(PIN_TO_DIGITAL(pin+7), a7);
#endif
}
