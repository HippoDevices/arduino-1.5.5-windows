...
void FirmataClass::USBProcessinput(byte * data)
{
	byte USBcmd; 
	USBcmd            =  data[0];
	if((data[1]<128)&&(data[2]<128))
	{
		storedInputData[1]=  data[1];   //LSB
		storedInputData[0]=  data[2];   //MSB
	}else
	{
		storedInputData[1]=  0;   //LSB
		storedInputData[0]=  0;   //MSB
	}

	
	if(USBcmd < 0xF0) 
	{
	  multiByteChannel = USBcmd & 0x0F;
      USBcmd = USBcmd & 0xF0;    
    } 
	  ...
	  case REPORT_ANALOG:
        if(currentReportAnalogCallback)
          (*currentReportAnalogCallback)(multiByteChannel,storedInputData[1]);
        break;
      case REPORT_DIGITAL:
    ...
    if( (waitForData==0) && executeMultiByteCommand ) { // got the whole message
	 ...
	 case Motor_MESSAGE:
	 	...
	 	if(currentMotorCallback){
			pinMode(41,OUTPUT);
			digitalWrite(41,1);
			(*currentMotorCallback)(storedInputData[1] ,storedInputData[0]);
        }
        break;
...
void FirmataClass::sendAnalog(byte pin, int value) 
{
  ...
  FirmataSerial->write(pin );
  sendValueAsTwo7bitBytes(value);
  FirmataSerial->write(1);  // Tx bits 7-13
  FirmataSerial->write(1);  // Tx bits 7-13
}
...
void FirmataClass::sendDigitalPort(byte portNumber, int portData)
{
  ...
  FirmataSerial->write(portNumber);
...
void FirmataClass::attach(byte command, callbackFunction newFunction)
{
  ...
  case Motor_MESSAGE: currentMotorCallback = newFunction; break;
  }
}
